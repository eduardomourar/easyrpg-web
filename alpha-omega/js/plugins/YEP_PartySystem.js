...
Window_PartyMenuCommand.prototype.makeCommandList = function() {
    ...
    this.addRemoveCommand(); this.addRevertCommand();
...
Window_PartyList.prototype.drawRestrictions = function(actor, rect) {
    ...
    if (actor._locked) if (actor._required) {
...
Window_PartyDetail.prototype.refresh = function() {
    ...
    this.contents.clear();
    this. drawActorHP(); this.calculateAvailableLines();
...
Window_PartyDetail.prototype.drawActorHP = function() {
    var w = this.width;
    var h = this.height;
    var y = 0;
    var padding = 0;
    var xpad = padding;
    var width = w - 330;
    h = Window_Base._lineHeight;
    this.drawActorHp(this._actor, xpad, y+47, width); this.drawActorTp(this._actor, xpad, y+85, width);
this.drawActorIcons(this._actor, xpad, y, width);};

Window_PartyDetail.prototype.calculateAvailableLines = function() {
    if (this._linesAvailable) return;
    this._linesAvailable = this.contents.height;
...
Window_PartyDetail.prototype.drawActorEquipsList = function(equips) {
    ...
    for (var i = 0; i < equips.length; ++i) {
      var equip = equips[i];
      if (!equip) break;
      if (i >= max - 1 && i < equips.length) this._lastSlot = true;
      if (this._lastSlot) {
